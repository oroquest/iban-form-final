...
// Validates token_iban / token_iban_expiry and returns readonly data for display.
...
exports.handler = async (event) => {
  try{
    ...
    // Build readonly pack: include human-relevant fields, exclude sensitive/token/iban fields
    const readonly = {};
    for (const [k,v] of Object.entries(props)) {
      const key = String(k || '');
      const lower = key.toLowerCase();
      if (lower.includes('iban') || lower.includes('token')) continue; // hide IBAN + token internals
      if (lower.includes('ip_verify') || lower.includes('agent_verify')) continue; // no debug meta
