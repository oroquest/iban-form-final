...
function parseInputs(event){
  const q=event.queryStringParameters||{}; let b={};
  ...
  let id=b.id??q.id, token=b.token??q.token, em=b.em??q.em, email=b.email??q.email, lang=(b.lang??q.lang??'de').toLowerCase();
  if((!id||!token||(!em&&!email))&&(event.headers.referer||event.headers.Referer)){ try{ const u=new URL(event.headers.referer||event.headers.Referer); const p=u.searchParams; id=id||p.get('id'); token=token||p.get('token'); em=em||p.get('em'); lang=lang||((p.get('lang')||'de').toLowerCase()); }catch{} }
...
exports.handler = async (event)=>{
  try{
    ...
    let email=emailIn; if(!email&&em) email=b64urlDecode(em)||em;
    ...
    const props = toObj((await r.json())?.Data?.[0]?.Data);
    ...
    // 2) get_contact by ID (server-side)
    ...
    const raw = await rc.text(); let data={}; try{ data=JSON.parse(raw) }catch{ return jres(rc.status||500, { ok:false, error:'Invalid JSON from get_contact', raw }) }
    if(!rc.ok) return jres(rc.status, { ok:false, error:`get_contact failed: ${rc.status}`, body:data });

    const readonly = data.readonly || data.data || data || {};
