...
// Accepts email/em + token + iban/iban_confirm, validates, writes to Mailjet contact fields
// Writes exactly: iban, ip_iban, timestamp_iban, agent_iban, token_iban_used_at; clears token_iban, token_iban_expiry.
...
exports.handler = async (event) => {
  try{
    ...
    if(ENFORCE_SINGLE_USE && String(props.token_iban_used_at||'').trim()!=='') {
    ...
    // Update bauen: GENAU die Felder wie vorgegeben
    const Data = [
    ...
    const u = await fetch(`https://api.mailjet.com/v3/REST/contactdata/${encodeURIComponent(email)}`, {
      method:'PUT',
      headers:{ Authorization: mjAuth, 'Content-Type':'application/json' },
      body: JSON.stringify({ Data })
    });
    if(!u.ok){ const t = await u.text(); return { statusCode:502, body:`Mailjet update failed: ${u.status} ${t}` }; }

    // Response/Redirect identisch zu verify
    const accept = (H['accept']||H['Accept']||'').toLowerCase();
    const wantsJSON = accept.includes('application/json') || (H['x-requested-with']||'').toLowerCase()==='fetch';
