// netlify/functions/issue_iban_token.js
...
// Optional env vars:
//   BASE_IBAN_URL            (default: https://iban.sikuralife.com/)
...
const BASE_IBAN_URL = (process.env.BASE_IBAN_URL || 'https://iban.sikuralife.com/').replace(/\/+$/,'');
...
async function setTokenInMailjet(email, token, expiresAtISO) {
  const updates = [
    ...
    { Name: 'token_iban_expiry',    Value: expiresAtISO },
    { Name: 'token_iban_used_at',   Value: '' } // reset single-use marker
  ];
...
exports.handler = async (event) => {
  try {
    ...
    const url = `${BASE_IBAN_URL}/?lang=${encodeURIComponent(lang)}&id=${encodeURIComponent(id)}&token=${encodeURIComponent(token)}&em=${encodeURIComponent(em)}`;
