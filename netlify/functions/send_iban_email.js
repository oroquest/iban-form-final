...
function pickTemplate(lang, category){ const l=pickLang(lang); const c=String(category||'direct').toLowerCase(); const g=TPL[l]||TPL.de; const k=c.includes('lawyer')?'lawyer':'direct'; return g[k]||g.direct||0; }
  ...
  const r = await fetchFn('https://api.mailjet.com/v3.1/send', {
    ...
    body: JSON.stringify({
      Messages:[{
        ...
        To:[{ Email: toEmail }],
        TemplateID:Number(templateId), TemplateLanguage:true, Variables:variables||{}
      }]
    })
  });
...
exports.handler = async (event)=>{
  try{
    ...
    const expiresISO = iso(addDays(new Date(), EXPIRY_DAYS));

    const contactId = await mjGetContactIdByEmail(email);
    await mjUpdateContactDataById(contactId, { token_iban:token, token_iban_expiry:expiresISO, token_iban_used_at:'' });
    ...
    return jres(200, { ok:true, templateId: templateId||0, expiresAt: expiresISO, url });
  }catch(err){
    return dbg(500, err);
  }
};
